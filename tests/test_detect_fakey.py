...
import pandas as pd
import pytest
from stockbee.detectors import detect_fakey
...
def test_if_fakey_detector_inputs_and_outputs_dataframe():
    ...
    assert type(detect_fakey(dataframe_out)) is pd.DataFrame

def test_if_pinbar_detector_returns_dataframe_with_column_fakey_added():
    dataframe_out = detect_fakey(pd.DataFrame([]))
...
def test_custom_pinbar_labels():
    dataframe_out = detect_fakey(pd.DataFrame([]), labelBuy="FakeBuy", labelSell="FakeSell")
...
def test_if_pinbar_column_is_0s_and_1s(dataframe_with_test_data):
    inputDataFrame = detect_fakey(dataframe_with_test_data)
...
def test_if_any_spare_columns_remain(empty_dataframe):
    inputDataFrame = detect_fakey(empty_dataframe)
